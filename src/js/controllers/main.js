...
function mainController($scope, $state, LoginFactory, DashboardFactory, SelectClassFactory, TakeAttendanceFactory, $rootScope, toastr) {
    ...
    $scope.getAllFeesKeywords = function() {
        DashboardFactory.getAllFeesKeywords(LoginFactory.loggedInUser.CollegeId)
            .then(function(success) {
                $scope.feesKeywords = [];
                if (success.data.Code != "S001") {
                    toastr.warning('Could not fetch fees keywords. Please try later!');
                } else {
                    $scope.feesKeywords = success.data.Data[0];
                }
            }, function(error) {
                toastr.error(error);
            });
    ...
    $scope.getAllSubjectsForUser = function() {
        SelectClassFactory.getAllSubjectsForUser($scope.loggedInUser.Id)
            .then(function(success) {
                if (success.data.Code != "S001") {
                    toastr.info('There are no subjects assigned to this user');
                } else {
                    $scope.subjects = success.data.Data;
                    $scope.selected.subject = $scope.subjects[0];
                    SelectClassFactory.selected.subject = angular.copy($scope.selected.subject);
    ...
    $scope.subjectSelected = function() {
        ...
        SelectClassFactory.getAllClassesForSubject($scope.selected.subject.Id, $scope.loggedInUser.Id, $scope.selected.subject.IsElective)
            .then(function(success) {
                $scope.classes = [];
                if (success.data.Code != "S001") {
                    ...
                    SelectClassFactory.selected.subject = angular.copy($scope.selected.subject);
                    SelectClassFactory.selected.class = angular.copy($scope.selected.class);
    ...
    $scope.getAllStudentsInClass = function() {
        TakeAttendanceFactory.getAllStudentsInClass(LoginFactory.loggedInUser.CollegeId, $scope.selected.subject.Id, $scope.selected.class.Id, $scope.selected.subject.IsElective)
            .then(function(success) {
                if (success.data.Code != "S001") {
                    toastr.info('There are no students in this class');
                } else {
                    SelectClassFactory.Students = [];
                    SelectClassFactory.ClassIds = [];
                    SelectClassFactory.SubjectIds = [];
                    SelectClassFactory.Students = success.data.Data;
                    SelectClassFactory.ClassIds = $scope.getUniqueIds(SelectClassFactory.Students, 'ClassId');
                    if ($scope.selected.subject.IsElective == "true") {
                        SelectClassFactory.SubjectIds = $scope.getUniqueIds(SelectClassFactory.Students, 'NormalSubjectId');
                    } else {
                        SelectClassFactory.SubjectIds.push($scope.selected.subject.Id);
                    }
                    if ($state.current.name != 'app.main') {
                        $state.go('app.main');
                    }
                    $rootScope.$broadcast('subjectsLoaded');
                }
            }, function(error) {
                toastr.error(error);
            });
    ...
    $scope.collegeSelected = function(college) {
        LoginFactory.loginCollege(college, LoginFactory.loginType);
    };

    $scope.getAllKeywords();
    $scope.getAllFeesKeywords();
